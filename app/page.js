...
const PLATFORM_INFO = {
  venmo: {
    label: 'Venmo',
    color: '#008CFF',
    inputLabel: 'Venmo username, phone, or email',
    inputPlaceholder: 'e.g. jerin-v or (713) 555-0123',
    helpText: 'Open Venmo → tap your profile icon (top-left) → your username is shown under your name. You can also use your phone number or email.',
  },
  paypal: {
    label: 'PayPal',
    color: '#0070BA',
    inputLabel: 'PayPal.me username OR email/phone',
    inputPlaceholder: 'e.g. jerinv or jerin@email.com',
    helpText: 'If you have a PayPal.me link, use just the username part (paypal.me/YOUR_USERNAME). If not, use the email or phone on your PayPal account — payers will see it as instructions to send manually.',
  },
  cashapp: {
    label: 'Cash App',
    color: '#00D632',
    inputLabel: 'Cash App $cashtag (without the $)',
    inputPlaceholder: 'e.g. jerinv',
    helpText: 'Open Cash App → tap your profile icon (top-right) → your $cashtag is shown at the top. Enter it without the $ sign.',
  },
  zelle: {
    label: 'Zelle',
    color: '#6D1ED4',
    inputLabel: 'Phone number or email registered with Zelle',
    inputPlaceholder: 'e.g. (713) 555-0123 or jerin@email.com',
    helpText: 'This is the phone or email you enrolled with in your banking app. Payers will see this and send via their own bank — there\'s no direct link for Zelle.',
  },
};

const M = { venmo: '#008CFF', paypal: '#0070BA', cashapp: '#00D632', zelle: '#6D1ED4' };
...
const Input = ({ label, hint, ...p }) => <label className="block">{label && <span className="text-[10px] font-bold text-stone-500 mb-1.5 block uppercase tracking-wider">{label}</span>}<input {...p} className={`w-full bg-stone-900 border border-stone-700 rounded-lg px-3.5 py-2.5 text-stone-100 text-sm placeholder:text-stone-600 focus:outline-none focus:border-emerald-500/50 ${p.className || ''}`} />{hint && <p className="text-stone-600 text-[10px] mt-1">{hint}</p>}</label>;
  ...
  return (
    <div className="min-h-screen bg-stone-950 flex flex-col items-center justify-center p-6" style={{ backgroundImage: 'radial-gradient(ellipse at 50% 0%, rgba(16,185,129,0.06) 0%, transparent 60%)' }}>
      <div className="mb-8 text-center">
        ...
        <p className="text-stone-600 text-sm max-w-xs mx-auto">One link for your payment info. Share it with anyone who pays you.</p>
      </div>
      <Card className="w-full max-w-sm space-y-4">
        <h2 className="text-base font-bold text-stone-200 text-center">Get your free payment link</h2>
      ...
      <div className="mt-10 max-w-sm mx-auto text-center">
        <p className="text-stone-600 text-xs uppercase tracking-wider font-bold mb-4">How it works</p>
        <div className="grid grid-cols-3 gap-4">
          <div><p className="text-2xl mb-1">🔗</p><p className="text-stone-400 text-xs font-semibold">Add your Venmo, PayPal, etc</p></div>
          <div><p className="text-2xl mb-1">📱</p><p className="text-stone-400 text-xs font-semibold">Share your link</p></div>
          <div><p className="text-2xl mb-1">💸</p><p className="text-stone-400 text-xs font-semibold">Get paid in one tap</p></div>
        </div>
      </div>
    </div>
  );
...
function Setup({ user, supabase, onDone }) {
  ...
  const create = async () => {
    ...
    await supabase.from('workers').insert({ user_id: user.id, name: name.trim(), slug: makeSlug(name), email: user.email, payment_methods: [], preferred_method: null });
    setLoading(false); onDone();
  };
  return (
    <div className="min-h-screen bg-stone-950 flex flex-col items-center justify-center p-6" style={{ backgroundImage: 'radial-gradient(ellipse at 50% 0%, rgba(16,185,129,0.06) 0%, transparent 60%)' }}>
      <Card className="w-full max-w-sm space-y-5">
        <div className="text-center"><div className="w-12 h-12 rounded-xl bg-emerald-500 flex items-center justify-center mx-auto mb-3"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg></div><h2 className="text-xl font-bold text-stone-100">Create your GigTab</h2><p className="text-stone-500 text-sm mt-1">10 seconds. You'll get a shareable payment link.</p></div>
        <Input label="What's your name?" value={name} onChange={e => setName(e.target.value)} placeholder="Your full name" />
...
function WorkerDash({ worker, supabase, onUpdate, signOut }) {
  ...
  const addMethod = () => { if (!nm.handle) return; const u = [...methods, { ...nm, id: Math.random().toString(36).substr(2,9) }]; setMethods(u); if (!pref) setPref(nm.type); setAdding(false); setShowHelp(false); setNm({ type: 'venmo', handle: '' }); save({ payment_methods: u, preferred_method: pref || nm.type }); };
  const updateJob = async (id, u) => { await supabase.from('jobs').update(u).eq('id', id); setJobs(prev => prev.map(j => j.id === id ? { ...j, ...u } : j)); };
  const submitHours = (jobId) => { if (!hrs) return; updateJob(jobId, { hours_logged: parseFloat(hrs), status: 'completed' }); setLoggingJob(null); setHrs(''); };
  ...
  const platformInfo = PLATFORM_INFO[nm.type];
      ...
      <main className="max-w-2xl mx-auto px-4 py-5">
        ...
        <div className="bg-emerald-950/30 border border-emerald-700/30 rounded-xl p-4 mb-5">
          <div className="flex items-center justify-between mb-2">
            ...
            {methods.length === 0 && <Badge color="yellow">Add payment method below</Badge>}
          </div>
        ...
        {tab === 'jobs' && (
          <div className="space-y-5">
            {jobs.length === 0 && <Card className="text-center py-8"><p className="text-3xl mb-3">📋</p><p className="text-stone-300 font-semibold mb-1">No jobs yet</p><p className="text-stone-500 text-xs max-w-xs mx-auto mb-4">When someone uses your link to assign you work or pay you, it'll show up here.</p><Btn onClick={copy}>{showCopied ? 'Copied!' : 'Copy My Link'}</Btn></Card>}
            ...
            {pending.length > 0 && <div><p className="text-amber-400 text-[10px] font-bold uppercase tracking-wider mb-2">New Invitations ({pending.length})</p>{pending.map(j => (
              <Card key={j.id} className="mb-3">
                ...
                <div className="flex gap-2 mt-3"><Btn onClick={() => updateJob(j.id, { status: 'accepted' })}>Accept</Btn><Btn onClick={() => updateJob(j.id, { status: 'declined' })} v="ghost">Decline</Btn></div>
              </Card>
            ))}</div>}

            {active.length > 0 && <div><p className="text-sky-400 text-[10px] font-bold uppercase tracking-wider mb-2">Active</p>{active.map(j => (
              <Card key={j.id} className="mb-3">
                <p className="text-stone-200 font-semibold">{j.title}</p>
                <p className="text-stone-500 text-xs">{j.payer_name}{j.job_date ? ` · ${fmtDate(j.job_date)}` : ''}{j.hourly_rate ? ` · ${fmt(j.hourly_rate)}/hr` : ''}</p>
                {loggingJob === j.id ? (
            ...
            {waiting.length > 0 && <div><p className="text-violet-400 text-[10px] font-bold uppercase tracking-wider mb-2">Awaiting Approval</p>{waiting.map(j => (
        ...
        {tab === 'profile' && (
          <div className="space-y-5">
            <div><h3 className="text-stone-200 font-bold text-sm mb-1">Payment Methods</h3><p className="text-stone-500 text-xs">These show on your public GigTab link. Anyone you share it with can use them to pay you.</p></div>
            ...
            {methods.length === 0 && !adding && (
              <Card className="text-center py-6 border-amber-800/30">
                ...
                <p className="text-stone-500 text-xs mb-4">Your GigTab link won't work until you add at least one.</p>
              ...
              <Card key={m.id}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-3 h-3 rounded-full" style={{ backgroundColor: M[m.type] }} />
                    <div>
                      <p className="text-stone-200 text-sm font-semibold">{PLATFORM_INFO[m.type]?.label || m.type}</p>
                      <p className="text-stone-500 text-xs font-mono">{m.handle}</p>
                    </div>
            ...
            {adding ? (
              <Card className="space-y-4">
                ...
                <Input
                  label={platformInfo?.inputLabel || 'Username or ID'}
                  value={nm.handle}
                  onChange={e => setNm({ ...nm, handle: e.target.value })}
                  placeholder={platformInfo?.inputPlaceholder || ''}
                />

                <button onClick={() => setShowHelp(!showHelp)} className="text-emerald-400 text-xs font-medium cursor-pointer hover:text-emerald-300">
                  {showHelp ? 'Hide help' : `Where do I find my ${platformInfo?.label} info?`}
                </button>

                {showHelp && (
                  <div className="bg-emerald-950/20 border border-emerald-800/20 rounded-lg px-3.5 py-3">
                    <p className="text-stone-300 text-xs leading-relaxed">{platformInfo?.helpText}</p>
                  </div>
                )}

                <div className="flex gap-2">
                  <Btn onClick={addMethod} disabled={!nm.handle}>Save</Btn>
                  <Btn onClick={() => { setAdding(false); setShowHelp(false); }} v="ghost">Cancel</Btn>
                </div>
              </Card>
            ) : methods.length > 0 && (
              <button onClick={() => setAdding(true)} className="text-emerald-400 text-sm font-semibold cursor-pointer hover:text-emerald-300">+ Add Another Payment Method</button>
            )}

            <Card>
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <Input label="Phone number (optional)" value={phone} onChange={e => { setPhone(e.target.value); setPhoneSaved(false); }} placeholder="(713) 555-0123" />
                </div>
                <button onClick={savePhone}
                  className={`text-xs font-bold px-4 py-2.5 rounded-lg cursor-pointer transition-colors ${phoneSaved ? 'bg-emerald-500/20 text-emerald-400' : 'bg-stone-800 text-stone-300 hover:bg-stone-700'}`}>
        ...
        {tab === 'earnings' && (
          <div className="space-y-5">
            <div><h3 className="text-stone-200 font-bold text-sm mb-1">Earnings ({yr})</h3><p className="text-stone-500 text-xs">Tracked per payer. A 1099 form is required if a single payer pays you $2,000+ in a year.</p></div>
            {Object.keys(byPayer).length === 0 && <Card className="text-center py-6"><p className="text-stone-500 text-sm">No payments recorded yet.</p></Card>}
            {Object.entries(byPayer).sort((a, b) => b[1] - a[1]).map(([payer, total]) => (
              <Card key={payer}>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="flex items-center gap-2"><span className={`font-bold text-sm ${total >= 2000 ? 'text-red-400' : 'text-stone-300'}`}>{fmt(total)}</span>{total >= 2000 && <Badge color="red">1099</Badge>}</div>
                </div>
                <div className="h-1.5 bg-stone-800 rounded-full overflow-hidden"><div className={`h-full rounded-full ${total >= 2000 ? 'bg-red-500' : 'bg-emerald-500'}`} style={{ width: `${Math.min(100, (total / 2000) * 100)}%` }} /></div>
                <p className="text-stone-700 text-[10px] mt-1 text-right">{total >= 2000 ? `$${(total - 2000).toFixed(2)} over threshold` : `$${(2000 - total).toFixed(2)} to $2,000`}</p>
              </Card>
            ))}
            <div className="bg-sky-950/20 border border-sky-800/20 rounded-xl px-3.5 py-2.5"><p className="text-sky-400 text-xs font-bold">$2,000 threshold is per payer (2026 law)</p><p className="text-stone-500 text-[10px]">Income is taxable even below this. This just tracks when a 1099 is required.</p></div>
          </div>
        )}
      </main>
    </div>
...
export default function GigTabApp() {
  ...
  const loadW = async (u) => {
    ...
    if (u.email) {
      ...
      if (w2) { await supabase.from('workers').update({ user_id: u.id }).eq('id', w2.id); setWorker({ ...w2, user_id: u.id }); setLoading(false); return; }
    }
  ...
  if (worker) return <WorkerDash worker={worker} supabase={supabase} onUpdate={() => loadW(user)} signOut={signOut} />;
  return null;
}
